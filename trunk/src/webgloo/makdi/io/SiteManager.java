...
public class SiteManager {
    ...
    public void store(IProfile profile) throws Exception {
        ...
        List<Keyword> newKeywords = KeywordsManager.loadNewKeywords();
        //java.sql.Connection connection = DBConnection.getConnection();
        //DBManager.storeKeywords(connection, KeywordsManager.loadNewKeywords());
        ...
        for (String keyword : keywords) {
            ...
            //get summary
            List<IData> items = profile.getFrontPageDriver().run(keyword);
            for (IData item : items) {
                summary.append(item.toHtml());
            }
            
            //store summary and  content
            MyWriter.toConsole("\n**content is :: " + content.toString());
            MyWriter.toConsole("\n**summary is :: " + summary.toString());
            MyWriter.toConsole("\n** end process :: " + keyword);
            break ;
